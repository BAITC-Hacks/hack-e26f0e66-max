...
> | | Key needed? |
> |---|---|
> | **A — run the whole thing from scratch, with the AI agents** | ✅ **Yes — an OpenAI API key is required.** Put it in `.env` before you start. |
> | **B — just look at the results produced during the hackathon** | ❌ **No.** No key, no account, no network, nothing to configure. |
> | **C — recompute everything with no AI at all** | ❌ **No.** Produces the same three CSVs, the same roles and the same ranking. |
>
> If you have no key and only want to see what the tool produces, go straight
> to **[option B](#b--just-look-at-the-results-produced-during-the-hackathon)** —
...
language is **Russian**; English and Қазақша are one click away, top right.
Selecting Қазақша shows a machine-translation notice.
