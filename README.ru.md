...
> Языки: **[English](README.md)** · Русский · [Қазақша](README.kk.md)
...
времени и стоимости → веб-интерфейс с публичной ссылкой.
...
| Интерфейс | `gradio` 5.50, `pyvis` 0.3.2 | веб-интерфейс и интерактивные карты сети, работающие без интернета |
| Агенты | `openai` (совместимый клиент), `python-dotenv` | ИИ-слой; **опционален во время выполнения** |
| Тесты | `pytest` | 68 тестов: контракт выгрузок, формулировки, агенты, интерфейс |
...
```bash
.venv/bin/python -m pytest -q      # ожидается: 68 passed
```
...
Стартовый язык — русский, переключение на қазақша и English — справа вверху.
